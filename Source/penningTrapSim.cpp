...
void PenningTrapSim::time_step_FE(double dt, bool inter){
    ...
    for (int i = 0; i < n; i++){
        ...
        std::tie(d_pos, d_vel) = trap.particles[i].derivative(trap.total_force(i, false, inter));
        d_pos_list.col(i) = dt * d_pos;
        d_vel_list.col(i) = dt * d_vel;
    }
    for (int i = 0; i < trap.particles.size(); i++){
        trap.particles[i].update(d_pos_list.col(i), d_vel_list.col(i));
    }
    trap.change_time(dt);
}
...
void PenningTrapSim::time_step_RK4(double dt, bool inter){
    ...
    // Calculate K1
    for (int i = 0; i < n; i++){
        std::tie(ddt_pos, ddt_vel) = trap.particles[i].derivative(trap.total_force(i, false, inter));
    ...
    for (int i = 0; i < trap.particles.size(); i++){
        trap.particles[i].update_temp(0.5*dt*k_1_pos.col(i), 0.5*dt*k_1_vel.col(i));
    }
    trap.change_time(0.5*dt);
    
    // Calculate K2
    for (int i = 0; i < n; i++){
        std::tie(ddt_pos, ddt_vel) = trap.particles[i].derivative(trap.total_force(i, true, inter));
    ...
    for (int i = 0; i < trap.particles.size(); i++){
        trap.particles[i].update_temp(0.5*dt*k_2_pos.col(i), 0.5*dt*k_2_vel.col(i));
    }
    ...
    for (int i = 0; i < trap.particles.size(); i++){
        trap.particles[i].update_temp(dt*k_3_pos.col(i), dt*k_3_vel.col(i));
    }
    trap.change_time(0.5*dt);
    ...
    k_pos = dt/6.*(k_1_pos + 2*k_2_pos + 2*k_3_pos + k_4_pos);
    k_vel = dt/6.*(k_1_vel + 2*k_2_vel + 2*k_3_vel + k_4_vel);

    // Do the actual time step
    for (int i = 0; i < trap.particles.size(); i++){
        trap.particles[i].update(k_pos.col(i), k_vel.col(i));
    }
}
